...
<head>
    ...
    <title>OurHome 雙人同住租屋品質與成本儀表板</title>
    ...
    <div class="container">
        <header>
            <div class="brand">
                ...
                <p>即時監控台北市 5 區（大安、中山、信義、松山、南港）特選房屋、雙人估算總成本與台電省錢神房</p>
            </div>
            <button class="refresh-btn" onclick="fetchHouses()">
                🔄 立即刷新
            </button>
        </header>
        ...
        <div class="controls-card">
            <input type="text" id="searchInput" class="search-box" placeholder="🔍 輸入關鍵字或路名搜尋 (如：忠孝東路、公館、大安、獨洗、台電...)" oninput="filterAndRender()">
    ...
    <script>
        ...
        function filterAndRender() {
            ...
            let filtered = allHouses.filter(h => {
                ...
                // 搜尋文字過濾
                if (searchText && !fullStr.includes(searchText)) {
                    return false;
                }

                // 標籤過濾
                if (currentFilter === 'taipower') {
                    return h.cost_info && h.cost_info.is_taipower;
                } else if (currentFilter === 'balcony') {
                    return h.couples_features && h.couples_features.some(f => f.includes('陽台'));
                } else if (currentFilter === 'washing') {
                    return h.couples_features && h.couples_features.some(f => f.includes('洗衣機'));
                } else if (['大安區', '中山區', '信義區', '松山區', '南港區'].includes(currentFilter)) {
                    return fullStr.includes(currentFilter.toLowerCase());
                }
            ...
            filtered.sort((a, b) => {
                ...
                const sizeA = parseSqft(a.size);
                const sizeB = parseSqft(b.size);

                if (sortVal === 'cost_asc') return costA - costB;
                if (sortVal === 'rent_asc') return rentA - rentB;
                if (sortVal === 'size_desc') return sizeB - sizeA;
        ...
        function renderGrid(houses) {
            ...
            container.innerHTML = houses.map(h => {
                ...
                const isTaipower = cost.is_taipower;
                const warnings = h.couples_warnings || [];
                const features = h.couples_features || [];

                const warningsHtml = warnings.map(w => `<div class="tag-warning">${w}</div>`).join('');
                const featuresHtml = features.map(f => `<span class="tag-feature">${f}</span>`).join('');

                return `
                    <div class="house-card">
                        <div class="card-header">
                            ${isTaipower 
                                ? '<div class="badge-taipower">✨ 雙人省錢神房 (台電計費)</div>' 
                                : '<div class="badge-normal">👩‍❤️‍👨 特選雙人同住物件</div>'}
                        ...
                        <a href="${h.link}" target="_blank" class="btn-link">
                            🔗 一鍵直達 591 房屋頁面 ➔
                        </a>
                    </div>
                `;
...
class DashboardRequestHandler(BaseHTTPRequestHandler):
    def log_message(self, format, *args):
        pass  # 隱藏一般 HTTP GET log 保持 console 乾淨
...
def run_dashboard_server(port: int = PORT):
    ...
    logger.info(f"🌐 OurHome Web 租屋儀表板已成功啟動！網址: http://localhost:{port}")
    try:
        httpd.serve_forever()
